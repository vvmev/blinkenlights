...
// relays are inverting
#define OUT_ON 0
#define OUT_OFF 1

//#define USE_PWM_BRIGHTNESS
...
int brightness = 0; // 0 is full brightness; any other value PWM up to PWMRANGE
...
static void all_off() {
  for (int i=4; i--; ) {
    ...
    digitalWrite(outputs[i], OUT_OFF);
  }
}
...
#if defined(USE_PWM_BRIGHTNESS)
    if (brightness == 0) {
#endif
      analogWrite(outputs[i], 0);
      digitalWrite(outputs[i], aspects[aspect][i] ? OUT_ON : OUT_OFF);
#if defined(USE_PWM_BRIGHTNESS)
    } else {
...
void handleRoot() {
  ...
  message += button("a", aspect, 0, "Sv0");
  message += button("a", aspect, 1, "Sv2v");
  message += button("a", aspect, 2, "Sv2");
  message += button("a", aspect, 3, "Sv1") + "</p>";
